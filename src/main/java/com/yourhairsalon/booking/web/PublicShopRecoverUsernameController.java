...
@RequestMapping("/public/shop/{shopname}/username")
@Controller
public class PublicShopRecoverUsernameController {
    ...
    public ModelAndView index(
    		@PathVariable String shopname,Model uiModel) {
    	log.debug("ENTERED INDEX OF PublicShopRecoverUsernameController");
    	ModelAndView mav = new ModelAndView();
		mav.setViewName("public/shop/username/index");	
    	
    	mav.addObject("register", new RegisterForm());
    	mav.addObject("successmessage","");
    	log.debug("EXITING INDEX OF PublicShopRecoverUsernameController");
        return mav;
    }
	@RequestMapping(method = RequestMethod.POST)
    public ModelAndView recoverUsername(@PathVariable String shopname,@Valid RegisterForm register,  BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
	...
	private boolean sendMessage(RegisterForm regform){
		try{
			...
			if (!email.equalsIgnoreCase("anonymous") && !email.equalsIgnoreCase("")) {
				...
				// find client by communication email
				TypedQuery<Communications> comms = Communications.findCommunicationsesByCommunication_valueEquals(email);
				log.debug("the returned results for looking for "+ email + " is a size of: " + comms.getResultList().size());
				if(comms.getResultList().size() == 1){
					Communications thiscomm = comms.getResultList().get(0);
					String username = thiscomm.getPerson().getUsername();
