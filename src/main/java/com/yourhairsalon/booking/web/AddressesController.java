...
public class AddressesController {
    ...
    public ResponseEntity<String> showJson(@PathVariable("id") Long id) {
        Addresses addresses = Addresses.findAddresses(id);
    ...
    public ResponseEntity<String> listJson() {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/text; charset=utf-8");
        return new ResponseEntity<String>(Addresses.toJsonArray(Addresses.findAllAddresseses()), headers, HttpStatus.OK);
    }

	@RequestMapping(method = RequestMethod.POST, headers = "Accept=application/json")
    public ResponseEntity<String> createFromJson(@RequestBody String json) {
        Addresses.fromJsonToAddresses(json).persist();
        HttpHeaders headers= new HttpHeaders();
    ...
    public ResponseEntity<String> updateFromJson(@RequestBody String json) {
        HttpHeaders headers= new HttpHeaders();
        headers.add("Content-Type", "application/text");
        if (Addresses.fromJsonToAddresses(json).merge() == null) {
            return new ResponseEntity<String>(headers, HttpStatus.NOT_FOUND);
        }
    ...
    @ResponseBody
    public ResponseEntity<String> jsonFindAddressesesByPerson(@RequestParam("person") AbstractPerson person) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/text; charset=utf-8");
        return new ResponseEntity<String>(Addresses.toJsonArray(Addresses.findAddressesesByPerson(person).getResultList()), headers, HttpStatus.OK);
    }
    ...
    public String show(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("addresses", Addresses.findAddresses(id));
    ...
    public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("addresseses", Addresses.findAddressesEntries(page == null ? 0 : (page.intValue() - 1) * sizeNo, sizeNo));
            float nrOfPages = (float) Addresses.countAddresseses() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("addresseses", Addresses.findAllAddresseses());
        }
    ...
    public String delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        Addresses.findAddresses(id).remove();
        uiModel.asMap().clear();
    ...
    public String findAddressesesByPersonForm(Model uiModel) {
        uiModel.addAttribute("abstractpeople", AbstractPerson.findAllAbstractpeople());
        return "addresseses/findAddressesesByPerson";
    }

	@RequestMapping(params = "find=ByPerson", method = RequestMethod.GET)
    public String findAddressesesByPerson(@RequestParam("person") AbstractPerson person, Model uiModel) {
        uiModel.addAttribute("addresseses", Addresses.findAddressesesByPerson(person).getResultList());
        return "addresseses/list";
    }
}
